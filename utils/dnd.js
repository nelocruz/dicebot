...
/** new **/
function rollD20(nickname, bonus, options) {
    var result = '';
    const hasAdv = (options || '').indexOf('adv') > -1;
    const hasDis = !hasAdv && (options || '').indexOf('dis') > -1;
    ...
    const value = (hasAdv ? sorted[1] : hasDis ? sorted[0] : rolls[0]) + bonus;
    result = `:game_die: ${nickname}:`;
    result += (hasAdv ? 'd20(adv) **[' + rolls[0] + ', ' + rolls[1] + ']**' : hasDis ? 'd20(dis) **[' + rolls[0] + ', ' + rolls[1] + ']**' : 'd20 **[' + rolls[0] + ']**');
    result += (bonus >= 0 ? ' **+' + bonus : ' **-' + (bonus * -1)) + '** = `' + value + '`';
    return result;
}
